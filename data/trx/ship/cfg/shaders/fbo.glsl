...
#define DITHER_MODE_OFF    0
#define DITHER_MODE_8_BIT  1
#define DITHER_MODE_15_BIT 2

// Levels of a 3-3-2 bit color, the arrangement 8-bit displays gave the
// channels, and of the 5-5-5 the PlayStation framebuffer holds.
const vec3 DITHER_STEPS_8_BIT = vec3(7.0, 7.0, 3.0);
const vec3 DITHER_STEPS_15_BIT = vec3(31.0, 31.0, 31.0);
...
void main(void) {
    ...
    if (uDitherMode != DITHER_MODE_OFF) {
        vec3 steps = uDitherMode == DITHER_MODE_15_BIT ? DITHER_STEPS_15_BIT
                                                       : DITHER_STEPS_8_BIT;
        outColor.rgb = dither(
            outColor.rgb, vertTexCoords * vec2(textureSize(uTex0, 0)), steps);
    }
}
