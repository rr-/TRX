...
api.type("items.Item", {
  ...
  fields = {
    ...
    object_id = {
      ...
      type = "integer",
      writable = false,
      description = "The item's object type. Compare against `trx.catalog.objects`.",
    },
    status = {
      from = "status",
      type = "integer",
      description = "Item status. Compare against `trx.items.Status`.",
    },
    flags = {
      from = "flags",
      type = "integer",
      description = "Trigger-related flag bits.",
    },
    timer = {
      from = "timer",
      type = "integer",
...
local function search(query, first_only)
  local found = first_only and nil or {}
  ...
  return first_only and nil or found
end
...
-- trx.items holds the registered functions; the metatable adds indexing and the
-- length operator on top.
rawset(trx.items, "Status", raw.Status)
