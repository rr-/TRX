...
def resolve_exe_from_artifact(artifact_zip: Path, temp_root: Path) -> Path:
    unpack_dir = temp_root / "windows_artifact"
    unpack_dir.mkdir(parents=True, exist_ok=True)
    with zipfile.ZipFile(artifact_zip) as handle:
        handle.extractall(unpack_dir)

    for path in sorted(unpack_dir.rglob("TRX.exe")):
        if path.is_file():
            return path
    raise FileNotFoundError(
        f"TRX.exe not found in windows artifact: {artifact_zip}"
    )
...
def main() -> None:
    ...
    if args.artifact.is_dir():
        if args.no_zip:
            raise ValueError("--no-zip cannot be used with a directory artifact")
        create_zip_from_dir(args.artifact, output_path)
