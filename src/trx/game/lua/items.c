...
static int M_L_Spawn(lua_State *const L)
{
    ...
    item->pos = pos;
    item->rot.y = luaL_optinteger(L, 3, 0);
...
void LUA_CreateItems(lua_State *const L)
{
    ...
    lua_newtable(L);
    lua_pushinteger(L, IS_INACTIVE);
    lua_setfield(L, -2, "INACTIVE");
    lua_pushinteger(L, IS_ACTIVE);
    lua_setfield(L, -2, "ACTIVE");
    lua_pushinteger(L, IS_DEACTIVATED);
    lua_setfield(L, -2, "DEACTIVATED");
    lua_pushinteger(L, IS_INVISIBLE);
